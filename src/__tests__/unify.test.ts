import {
  Term, Var, Const, isConst, isEqual, eliminateVariables, unify,
} from '../unify';
...
it('test isEqual', () => {
  ...
  expect(isEqual(new Term([new Const(42), new Var('foo')]), new Term([new Const(42), new Var('foo')]))).toBe(true);
  expect(isEqual(new Term([new Const(42), new Var('foo')]), new Term([new Var('foo'), new Const(42)]))).toBe(false);
});
...
it('test term resolveWith', () => {
  ...
  expect(isEqual(term,
    new Term([
      new Term([new Const(42), new Const(42)]),
      new Const(42),
    ]))).toBe(true);
});
...
it('test isConst', () => {
  ...
  expect(isConst(new Term([new Term([new Const(42), new Var('foo')])]))).toBe(false);
});
...
it('test eliminateVariables', () => {
  ...
  expect(isEqual(
    eliminateVariables(new Term([new Const(42), new Var('foo'), new Var('bar')]), new Map([['foo', new Const(42)]])),
...
it('test unify c c', () => {
  expect(isEqual(
    unify(new Const(42), new Const(42))!,
    new Const(42),
  ));
...
it('test unify v t', () => {
  const dict = new Map();
  expect(isEqual(
    unify(new Var('foo'), new Term([]), dict)!,
    new Term([]),
  ));
...
it('test unify temp', () => {
  ...
  unify(new Term([new Var('b'), new Const(42)]), new Term([new Const(24), new Var('c')]), dict);

  expect(isEqual(dict.get('a')!, new Term([new Const(24), new Const(42)]))).toBe(true);
