...
type create_list_helper<Num, List extends ArrayLike<unknown>> =
  is_same<List['length'], Num> extends true
    ? List
...
type string_concat_helper<L, S extends string> = L extends [
  infer First,
  ...infer Rest,
]
...
export type tuple_to_union<T> =
  T extends ArrayLike<unknown> ? T[number] : never;

export type union_to_intersect<T> = (
  T extends unknown ? (_: T) => void : never
) extends (_: infer I) => void
  ? I
  : never;

type union_to_intersect_fn<T> = union_to_intersect<
  T extends unknown ? (_: T) => void : never
>;

export type union_last<T> =
  union_to_intersect_fn<T> extends (_: infer Last) => void ? Last : never;

export type union_to_tuple<T> =
  is_same<T, never> extends true
    ? []
...
type gs_helper<choices, subset, result> =
  concat<result, [subset]> extends infer r
...
export type intersect_to_tuple<I> =
  get_subsets<union_to_tuple<I>> extends infer subsets
