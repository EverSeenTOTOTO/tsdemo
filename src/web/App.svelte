<script>
  ...
  cameraFolder.add(cameraSetting, "fov", 1, 180).onChange(() => {
    cameraSetting.fov = cameraSetting.fov;
  });
  cameraFolder.add(cameraSetting, "aspect", 0.1, 10).onChange(() => {
  ...
  cameraFolder.add(cameraSetting, "far", 0.1, 10).onChange(() => {
    cameraSetting.far = cameraSetting.far;
  });
  cameraFolder.add(cameraSetting.position, "x", -10, 10).onChange(() => {
    cameraSetting.position.x = cameraSetting.position.x;
  });
  cameraFolder.add(cameraSetting.position, "y", -10, 10).onChange(() => {
    cameraSetting.position.y = cameraSetting.position.y;
  });
  cameraFolder.add(cameraSetting.position, "z", -10, 10).onChange(() => {
  ...
  lightFolder.addColor(lightSetting, "color").onChange(() => {
    lightSetting.color = lightSetting.color;
  });
  lightFolder.add(lightSetting, "intensity", 0, 2).onChange(() => {
    lightSetting.intensity = lightSetting.intensity;
  });
  lightFolder.add(lightSetting.position, "x", -5, 5).onChange(() => {
    lightSetting.position.x = lightSetting.position.x;
  });
  lightFolder.add(lightSetting.position, "y", -5, 5).onChange(() => {
    lightSetting.position.y = lightSetting.position.y;
  });
  lightFolder.add(lightSetting.position, "z", -5, 5).onChange(() => {
  ...
  const setupControls = ({detail}) => {
    ...
    controlsFolder.add(controls, "drag").onChange(() => {
      dragControls.enabled = controls.drag;
    });
    controlsFolder.add(controls, "orbit").onChange(() => {
      orbitControls.enabled = controls.orbit;
    });
  };
</script>
