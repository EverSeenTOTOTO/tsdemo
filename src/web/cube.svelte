<script>
  ...
  cubeFolder.addColor(cubeProps, "color").onChange((value) => {
    // explicitly call set
    cubeProps.color = cubeProps.color;
  });

  const rx = cubeFolder
    .add(cubeProps, "rx", 0, Math.PI * 2, 0.01)
    .onChange(() => {
      cubeProps.rx = cubeProps.rx;
    });
  const ry = cubeFolder
    .add(cubeProps, "ry", 0, Math.PI * 2, 0.01)
    .onChange(() => {
      cubeProps.ry = cubeProps.ry;
    });
  const rz = cubeFolder
    .add(cubeProps, "rz", 0, Math.PI * 2, 0.01)
    .onChange(() => {
  ...
  cubeFolder.add(cubeProps, "FPS", 10, 100).onChange(() => {
